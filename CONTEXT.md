...
| Recon pair (survey) | ✅ Built (PR #195, `design-recon-pair.md`); Ph0/Ph2 cleared, Ph1 walk owed | Mobile spoke: pulls taskings, surveys, offloads (set `SURVEY_FIXED_URL`+`SURVEY_TOKEN`); its own dashboard now has a Survey tab + patrol start/end and a wardrive index (needs this node's own `GUI_TOKEN` set) | Fixed server: `/api/tasking`+`/api/survey` on :8088, "Task survey" + Survey tab. `SURVEY_ENABLED` default off |
