...
| `modules/survey_coordinator.py` | `SurveyCoordinator` | Recon-pair survey logic lifted out of the orchestrator: the mobile matcher (`record_hits`), fixed-node tasking evidence + opt-in auto-task (`note_flagged_contact`), and the patrol-aware store-and-forward `sync_loop`. Owns the `SurveyStore`/`SurveySync`; guarded and inert when survey is off. The orchestrator holds it as `.survey` |
...
- **Offline OUI manufacturer lookup (#210):** `OUIDatabase` + module singleton `get_manufacturer(mac)` resolve a vendor from the Wireshark `manuf` file (`data/oui/manuf`, override `OUI_MANUF_PATH`; fetch via `scripts/fetch_oui.sh`) — lazy-loaded, thread-safe, longest-prefix match across MA-L/MA-M/MA-S (24/28/36-bit) blocks. Randomized MACs resolve to `""`/Unknown (a private address has no real vendor). `KismetModule` uses it as a fallback when Kismet's own `manuf` is empty/Unknown; BLE adverts (no Kismet manuf source) resolve straight through it.
- `get_randomization_vendor_hint(mac)` — randomized → `"Unknown"`; static → delegates to `get_manufacturer(mac)` (was `""` before #210)
...
- `KISMET_ACTIVE_WINDOW_SECONDS` (default `300`; `0` = opt out): devices whose Kismet `last_time` is older than this many seconds are excluded from `poll_devices()` — before the list reaches baseline learning, the entity store, or scoring. Kismet's device list is permanent, so without this filter every device ever heard in the session remains in every poll. This is wrong for **both** roles: on a **mobile** node the node's own GPS movement stamps departed devices with new positions, creating spurious "following" clusters (road-test 2026-06-20: 4,833 devices flagged, 39k alerts); on a **fixed** node a passer-by heard once gets its baseline hour-mask stamped with the current hour every subsequent poll it lingers, smearing a transient into an all-day "resident" and re-flagging departed devices post-freeze (#232 — verified 97% of devices heard <1h had baselines spanning ≥3 hours). The `0` default was a footgun; `300` keeps only recently-heard (present) devices. Mobile nodes may tighten to 90–120. **Note:** the fix only cleans *new* learning — a baseline learned under `0` is already smeared and must be re-learned (wipe `data/baseline.db`, re-run the window).
...
| `deploy/kismet-site-fixed-node.conf` | Fixed-node Kismet fragment (`enable_logging=false`); `install.sh` appends it to `kismet_site.conf` idempotently — cures the kismetdb fsync SOURCEERROR stall |
| `deploy/sdr-wedge-watch.sh` + `deploy/sdr-wedge-watch.cron` | SDR-wedge watchdog; `install.sh` installs the script under the pi user + a root cron that auto-`usbreset`s the RTL-SDR when readsb crash-loops on "SDR wedged" |
...
- **SDR handoff waits for a confirmed decoder release (honest-release, #216/#217):** an AIS/ACARS decoder that overruns its slice can keep the dongle open; starting readsb onto that still-busy device is the "SDR wedged / Device or resource busy" crash-loop. Both `_handoff_to` (mid-cycle) **and** `stop()` (shutdown) now check the release handshake's boolean result and refuse to acquire/restart readsb until the previous owner is confirmed released, keeping `coordinator.healthy` truthful. `_reconnect("sdr")` verifies `coordinator.healthy` after `start()` so a still-stuck SDR is reported as a failed reconnect (retry/backoff), not falsely marked recovered. A genuinely wedged dongle is cleared by the `sdr-wedge-watch` cron (usbreset), installed from the repo by `deploy/install.sh` (`deploy/sdr-wedge-watch.sh` + `.cron`).
- **entities.db is bounded so it can't crash-loop the node (#206/#207):** on a busy fixed node the observation history grew until a WAL-backed read stalled the poll loop past the systemd watchdog → SIGABRT restart loop. `EntityStore` now bounds it three ways — age retention, a hard row cap (`ENTITY_OBSERVATION_MAX_ROWS`), and a periodic WAL TRUNCATE checkpoint — plus an optional off-loop writer (`ENTITY_ASYNC_WRITES`). On SD storage at high device density the tighter profile in `.env.example` ("high-density fixed-node profile") is required; validated 45 h+ with the DB flat at its cap. Node-local safety nets: `pv-wal-watch` cron (proactive restart if the WAL runs away) and `pv-async-watch` (hourly soak log). Underlying ceiling is SD fsync latency (issue #211) — durable fix is a USB SSD.
